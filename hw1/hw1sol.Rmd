...
```{bash}
zcat /mnt/mimiciv/1.0/core/admissions.csv.gz | tail -n+2 | sort -u -t, -k1,1 | wc -l
#zcat /mnt/mimiciv/1.0/core/admissions.csv.gz | tail -n+2 | awk -F"," '!_[$1]++' | wc -l
```
...
```{bash}
zcat /mnt/mimiciv/1.0/core/admissions.csv.gz | awk -F ',' 'NR>1 {print $6}' | sort | uniq -c
```
```{bash}
zcat /mnt/mimiciv/1.0/core/admissions.csv.gz | awk -F ',' 'NR>1 {print $7}' | sort | uniq -c
```
```{bash}
zcat /mnt/mimiciv/1.0/core/admissions.csv.gz | awk -F ',' 'NR>1 {print $9}' | sort | uniq -c
```
```{bash}
zcat /mnt/mimiciv/1.0/core/admissions.csv.gz | awk -F ',' 'NR>1 {print $12}' | sort | uniq -c
```
...
time sleep 5
```
***`time sleep 5` delays for 5 seconds during the execution of any scrip.***
