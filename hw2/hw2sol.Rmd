...
#typeof(read.csv("/mnt/mimiciv/1.0/core/admissions.csv.gz"))
#typeof(read_csv("/mnt/mimiciv/1.0/core/admissions.csv.gz",
#                show_col_types = FALSE))
#typeof(fread("/mnt/mimiciv/1.0/core/admissions.csv.gz"))
```
***Using the `str` function, we can check the parsed data types of these three programs. `read.csv` generates a data frame; `read_csv` generates a tibble; `fread` generates a data table. All the generated files are lists***
...
```{r}
icustays_id <- select(icustays_tble, c("subject_id", "hadm_id"))
admissions_tble <- admissions_tble %>%
  inner_join(icustays_id, by = c("subject_id", "hadm_id"))
```
...
admissions_tble %>% 
  ...
  theme_bw()
```
***Extremely high number of admissions at midnight (12 am).***
...
```
***There is a higher proportion of male patients. The average anchor age of the patients is 63.5.***
...
labevents_tble %>% 
  group_by(itemid) %>%
  summarise(itemmean = mean(valuenum, na.rm = TRUE))
```
...
mimic_icu_cohort <- icustays_tble %>%
  ...
  mutate(thirty_day_mort = ifelse(deathtime - admittime < 30, 1, 0)) %>%
  mutate(thirty_day_mort = coalesce(thirty_day_mort, 0))
```
...
mimic_icu_cohort %>% 
  ...
  summarise(age_hospadmin = mean(anchor_age, na.rm = TRUE))

table(mimic_icu_cohort$thirty_day_mort, mimic_icu_cohort$itemid.x)
table(mimic_icu_cohort$thirty_day_mort, mimic_icu_cohort$itemid.y)
